...
import {
  WORK_CORRECTION_MOCK_MODEL,
  WORK_CORRECTION_PDF_FILE_NAME,
  type WorkCorrectionImageRef,
  type WorkCorrectionModel,
} from './workCorrectionModel';
...
function createImageEmbedder(pdfDoc: PDFDocument) {
  ...
  return async (ref: WorkCorrectionImageRef): Promise<RasterImage | null> => {
    if (ref == null) {
      return null;
    }
      ...
      cache.set(
        ...
        (async () => {
          try {
            ...
            console.warn('work-correction embed image failed:', error);
            return null;
          }
        })(),
      );
...
function drawDocumentHeader(
  ...
  const dateWidth = boldFont.widthOfTextAtSize(model.headerDate, HEADER_FONT_SIZE);
  page.drawText(model.headerDate, {
  ...
  const pageLabel = `${model.pageLabelPrefix}   หน้า ${pageIndex} / ${totalPages}`;
...
function drawInfoBox(
  ...
  // วาด text ก่อนเพื่อคำนวณ height จริง — rectangle (ไม่มี fill) วาดทีหลังโดยไม่ทับ text
  ...
  adv(drawLabelValue(page, `${toInspectLabel(model.pageLabelPrefix)}: `, model.openInspectDate, infoX, y, infoW, boldFont, regularFont));
  adv(drawLabelValue(page, 'เจ้าหน้าที่ตรวจสอบ: ', model.officerName, infoX, y, infoW, boldFont, regularFont));
  ...
  // const h3 = drawLabelValue(page, 'ผู้รับมอบอำนาจ: ', model.authorizedName, infoX, y, colW, boldFont, regularFont);
  // const h4 = drawLabelValue(page, 'Tel: ', model.authorizedPhone, infoX + colW, y, infoW - colW, boldFont, regularFont);

  // height จริงของ box = สูงสุดระหว่าง content กับ INFO_BOX_HEIGHT เดิม
  ...
  // วาด border + divider + plan image หลัง text (rectangle ไม่มี fill → ไม่ทับ text)
  ...
  if (planImage) {
    const frame = drawContainedImage(page, planImage, planX, INFO_BOX_TOP, PLAN_WIDTH, actualBoxHeight, mm(2));
...
function drawRow(
  ...
  if (captureAreaImage) {
    // รูปแคปบริเวณหมุด (defectCaptureArea) จาก get defect — มีหมุดในรูปแล้ว วาดตรง ๆ
...
function drawDottedWithValue(
  ...
  if (value) {
    ...
    if (firstDot >= 0 && lastDot >= firstDot) {
      ...
      // เมื่อ center ทั้ง template และ value ใน width เดียวกัน
      // ต้องการ template กว้างกว่า value อย่างน้อย 2*prefixW จึง value จะอยู่ใน bracket ได้
...
function drawSignatureBlock(
  ...
  columns.forEach((column) => {
    drawText(page, column.title, column.x, boxTop + mm(2), boldFont, BODY_FONT_SIZE, {
      // maxWidth: half,
    ...
    drawText(page, dateLabel, column.x, boxTop + mm(19), regularFont, SMALL_FONT_SIZE, {
      // maxWidth: half,
      maxWidth: column.width,
      align: 'center',
    });
    // drawDottedWithValue(page, DATE_VALUE_DOTS, dateValue, column.x, boxTop + mm(22.5), half, regularFont);
    drawDottedWithValue(page, DATE_VALUE_DOTS, dateValue, column.x, boxTop + mm(22.5), column.width, regularFont);
  });
...
export async function exportWorkCorrectionDocumentToPdf(
  model: WorkCorrectionModel = WORK_CORRECTION_MOCK_MODEL,
  fileName?: string,
) {
  ...
  // เก็บ page handle ไว้วาด header ทีหลังหลังรู้จำนวนหน้าจริง
  const pageHandles: { page: PDFPage; pageIndex: number }[] = [];

  let pageIndex = 1;
  let page = pdfDoc.addPage([PAGE_WIDTH, PAGE_HEIGHT]);
  pageHandles.push({ page, pageIndex });
  ...
  // ฉบับย่อ: บล็อกลายเซ็น "รับทราบรายการ" + (ปิดรอบ) บล็อก "ดำเนินการแล้วเสร็จ..." + หมายเหตุ
  const isCloseRound = model.variant === 'close';
  const footerHeight =
    mm(7 + 28) + // signature bar + box (รับทราบรายการ)
    (isCloseRound ? mm(4) + mm(7 + 28) : 0) + // ปิดรอบ: เพิ่มบล็อก "ดำเนินการแล้วเสร็จ..."
    mm(8) + // remarks gap
  ...
  // ปิดรอบ: บล็อกลายเซ็นที่สอง ใช้ helper เดียวกับ "รับทราบรายการ" (style เหมือนกันตามที่ผู้ใช้ขอ)
  // "ดำเนินการแล้วเสร็จ" = นับแถวสถานะ "ปิดรายการ", total = ทุกแถว → "x รายการ (x / total รายการ)"
  if (isCloseRound) {
    const completedCount = model.rows.filter((row) =>
      isWorkDetailStatusCompleted(row.status),
    ).length;
    const totalCount = model.rows.length;
    footerY += mm(4);
    footerY = drawSignatureBlock(
      page,
      footerY,
      `ดำเนินการแล้วเสร็จ และตรวจรับมอบแล้ว ${completedCount} รายการ (${completedCount} / ${totalCount} รายการ)`,
      'วันที่ตรวจรับรายการ',
  ...
  // วาด header หลังทุก page ถูกสร้างแล้ว — totalPages จึงถูกต้องเสมอ
  const totalPages = pdfDoc.getPageCount();
  for (const { page: p, pageIndex: idx } of pageHandles) {
    drawDocumentHeader(p, model, logo, boldFont, idx, totalPages);
  }

  const pdfBytes = await pdfDoc.save();
  await downloadPdf(pdfBytes, `${fileName?.trim() || WORK_CORRECTION_PDF_FILE_NAME}.pdf`);
}
